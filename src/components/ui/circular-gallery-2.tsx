...
function createTextTexture(
  ...
  // Extract the px size specifically (the font string may be a full shorthand
  // like "700 30px sans-serif", where a leading parseInt would grab the weight).
  const pxMatch = font.match(/(\d+(?:\.\d+)?)px/);
  const fontSizePx = pxMatch ? parseFloat(pxMatch[1]) : 30;
...
class Title {
  ...
  createMesh() {
    ...
    const textHeight = this.plane.scale.y * 0.24;
    const textWidth = textHeight * aspect;
    this.mesh.scale.set(textWidth, textHeight, 1);
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeight * 0.5 - 0.35;
