...
interface FluidParticlesBackgroundProps {
  ...
  className?: string;
  /**
   * This project has no dark-mode toggle, so instead of reading
   * `document.documentElement.classList.contains("dark")`, the color
   * scheme is passed explicitly. Defaults to true (white particles),
   * meant for placement over a dark section.
   */
  dark?: boolean;
}
...
export const FluidParticlesBackground = ({
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const scheme = dark ? COLOR_SCHEME.dark : COLOR_SCHEME.light;
